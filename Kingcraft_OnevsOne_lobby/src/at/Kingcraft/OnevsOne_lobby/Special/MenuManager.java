...
public class MenuManager {
	...
	{
		if(rankedMenus.get(p.getUniqueId()) == null)
		{
			rankedMenus.put(p.getUniqueId(), new RankedMenu(p));
		}
		
		return rankedMenus.get(p.getUniqueId());
	}
	
	public static void deleteRankedMenu(Player p)
	{
		if(rankedMenus.get(p.getUniqueId()) != null)
		{
			rankedMenus.remove(p.getUniqueId());
		}
	}
}
